...
### mgr Maciej Szupiluk
- Added two calibration tools, run once per camera model and saved to the database: how many frames per second each continuous-drive mode (cont-lo/mid/hi/hi+) actually shoots, and how the memory card's write speed/buffer capacity behaves under sustained shooting - replaces guessed numbers that didn't match real cameras
- Split the Timeline's "pause for settings change" bar into two separate zones: an amber ARM zone (time to actually change ISO/aperture/etc. between blocks) and a new gray WR (buffer-write) zone showing how long the camera is predicted to still be busy writing photos to the memory card - previously shown as one combined guess, now reflecting two genuinely different, independently measured causes of delay
- Reworked how the app predicts how full the camera's photo buffer is at any point in a sequence: instead of averaging a whole block's shots into one flat rate, it now walks through each individual shot's predicted timing (a bracket's shots are not evenly spaced - exposures get much longer toward the slow end) and tracks the buffer filling and draining shot by shot; also added a live "Buf" column next to each camera's status so this prediction can be visually compared against the camera's real behavior on video - still an early version, and the predicted numbers currently run low for very long continuous bursts; being refined further
- Fixed the camera server reporting a failed shot ("timeout") for every continuous-drive (Burst) block even when it fired every photo correctly - it was checking the wrong internal signal, one that always looks like a timeout for this kind of held-shutter shot
- Fixed the app disconnecting from the camera server (and skipping every following scheduled block) after a single long continuous-drive block - it was giving up and reporting an error to the operator while the server was still correctly holding the shutter in the background, using a wait time too short for how long that kind of block can legitimately take
